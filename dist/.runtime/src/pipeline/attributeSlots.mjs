//#region src/pipeline/attributeSlots.ts
...
const ATTRIBUTE_VALUE_QUERY_RE = /(?:什么|哪些|哪一个|哪种|是哪|是什么|用什么|使用什么|what|which|who|where|when|value|default|current)/iu;
function normalizeSlotText(value) {
	return value.replace(/([a-z0-9])([A-Z])/g, "$1_$2").normalize("NFKC").toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "");
}
...
function canonicalAttributeSlot(value) {
	const raw = value?.trim();
	if (!raw) return;
	const slot = normalizeSlotText(raw.split(/[.:/#|]+/u).map((entry) => entry.trim()).filter(Boolean).at(-1) ?? raw);
	if (!slot) return;
...
function semanticFactPredicatesForAttributeSlot(slot) {
	return [`has_${slot}`, `uses_${slot}`];
}
function attributeSlotFromPredicate(predicate) {
	return canonicalAttributeSlot(predicate);
}
function predicateMatchesAttributeSlots(predicate, requestedSlots) {
	const slot = attributeSlotFromPredicate(predicate);
	return Boolean(slot && requestedSlots.includes(slot));
}
function aliasMatchesNormalizedText(normalizedText, alias) {
	...
	if (/[\p{Script=Han}]/u.test(normalizedAlias)) return compactText.includes(compactAlias);
	const tokens = normalizedText.split("_").filter(Boolean);
	const aliasTokens = normalizedAlias.split("_").filter(Boolean);
	if (aliasTokens.length === 0 || tokens.length < aliasTokens.length) return false;
...
function requestedAttributeSlotsFromText(...values) {
	...
	for (const value of values) {
		...
		for (const slot of Object.values(CANONICAL_ATTRIBUTE_SLOT_ALIASES)) if (aliasMatchesNormalizedText(normalizedText, slot)) slots.add(slot);
		const aliases = Object.keys(CANONICAL_ATTRIBUTE_SLOT_ALIASES).sort((left, right) => right.length - left.length);
		for (const alias of aliases) if (aliasMatchesNormalizedText(normalizedText, alias)) slots.add(CANONICAL_ATTRIBUTE_SLOT_ALIASES[alias]);
	}
	return [...slots];
}
function queryAsksForAttributeValue(query) {
	return ATTRIBUTE_VALUE_QUERY_RE.test(query.normalize("NFKC"));
}
function attributeSlotContractHints(slots) {
	...
	for (const slot of slots) {
		...
		for (const [alias, canonical] of Object.entries(CANONICAL_ATTRIBUTE_SLOT_ALIASES)) if (canonical === slot) {
			hints.add(alias);
...
function attributeSlotAliasesForSlots(slots) {
...
//#endregion
export { attributeSlotAliasesForSlots, attributeSlotContractHints, attributeSlotFromPredicate, canonicalAttributeSlot, normalizeSemanticFactPredicate, predicateMatchesAttributeSlots, queryAsksForAttributeValue, requestedAttributeSlotsFromText, semanticFactPredicatesForAttributeSlot };
