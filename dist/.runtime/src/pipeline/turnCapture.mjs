import { normalizeText, normalizedTerms, stableHash } from "../support.mjs";
import { shouldSuppressCapturedMessage } from "./bootstrapFilter.mjs";
import { stripInjectedHistoricalBlock } from "../security/escaping.mjs";
import { readMessageText, stripInboundMetadata } from "./messageText.mjs";
import { isHeartbeatControlText } from "./heartbeatFilter.mjs";
...
function looksLikeRecallEcho(text, recalledTexts) {
	const lowered = normalizeText(text);
	if (lowered.length < 20) return false;
	for (const recalled of recalledTexts) {
		const normalized = normalizeText(recalled);
		if (normalized.length < 12) continue;
		if (lowered.includes(normalized) || normalized.includes(lowered)) return true;
		if (tokenOverlap(lowered, normalized) >= .55) return true;
	}
	return false;
}
...
function offsetObservedAt(baseObservedAt, index) {
	const baseMs = Date.parse(baseObservedAt);
	if (!Number.isFinite(baseMs)) return baseObservedAt;
	return new Date(baseMs + index).toISOString();
}
function captureAgentEndTurn(params) {
	const raw = [];
	for (const message of params.messages) {
		...
		const toolName = role === "tool" ? typeof entry.name === "string" ? entry.name : typeof entry.toolName === "string" ? entry.toolName : void 0 : void 0;
		if (role === "tool" && toolName && SELF_TOOL_RE.test(toolName)) continue;
		let text = readMessageText(entry.content).trim();
		if (!text) continue;
		if (role === "user") {
			text = stripInboundMetadata(stripInjectedHistoricalBlock(text));
			if (SYSTEM_BOILERPLATE_RE.test(text)) continue;
		} else text = stripInjectedHistoricalBlock(text);
		text = text.trim();
		if (!text) continue;
		if (isHeartbeatControlText(text)) continue;
		if (shouldSuppressCapturedMessage({
			role,
			content: text,
			toolName
		})) continue;
	...
	for (let index = 0; index < raw.length; index += 1) {
		...
		if (!looksLikeRecallEcho(combined, params.recalledTexts ?? []) && !looksLikeAssistantMemoryConfirmation(combined)) merged.push({
			role: "assistant",
			content: combined
		});
	}
	return merged.map((entry, index) => ({
		...
		scope: params.scope,
		sourceRef: `${entry.role}:${stableHash([
			params.agentId,
			params.sessionKey,
			params.turnId,
			String(index)
