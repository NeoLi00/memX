...
import {
  buildTaskSummaryEvidenceSet,
  taskSummaryMetadataFields,
  taskSummaryNeedsUpgrade,
  taskSummarySource,
  taskSummaryUpgradePriority,
} from "./taskSummary.js";
...
type RecentEvent = ReturnType<MemxStoreBundle["eventRepo"]["search"]>[number];
...
type PendingRelationConfirmation = {
  text: string;
  events: RecentEvent[];
  relation: MemoryCandidateRelationHint;
};

type StructuredConfirmationCollection = {
  pendingFacts: PendingFactConfirmation[];
  pendingRelations: PendingRelationConfirmation[];
  touchedFactKeys: Set<string>;
  touchedRelationKeys: Set<string>;
...
type ConsolidationBatchDecision = "confirm" | "defer" | "reject";
...
function shouldSkipLlmTaskSummaryUpgrade(ctx: MemoryOperationContext): boolean {
  return typeof ctx.runId === "string" && ctx.runId.startsWith("lme-replay:");
}

function eventStructuredHints(event: RecentEvent): MemoryCandidateStructuredHints | null {
  const metadata = objectRecord(event.metadataJson);
  const structured = objectRecord(metadata?.memxStructuredHints);
  return structured ? (structured as MemoryCandidateStructuredHints) : null;
}

function sortPendingGroups<T extends { events: RecentEvent[] }>(groups: T[]): T[] {
...
function summarizeMaintenanceCalls(
  audit: MemoryLlmBudgetAudit | undefined,
  labels: string[],
): MaintenanceCallSummary {
  const relevant = (audit?.calls ?? []).filter(
    (entry) => entry.stage === "maintenance_async" && labels.includes(entry.label),
  );
  return relevant.reduce<MaintenanceCallSummary>(
    (summary, entry) => ({
      callCount: summary.callCount + 1,
      estimatedPromptTokens: summary.estimatedPromptTokens + (entry.estimatedPromptTokens ?? 0),
      estimatedCompletionTokens:
        summary.estimatedCompletionTokens + (entry.estimatedCompletionTokens ?? 0),
...
function structuredEventEvidenceSummary(event: RecentEvent): string {
  const metadata = objectRecord(event.metadataJson);
  const temporalFacet = objectRecord(metadata?.memxTemporalFacet);
  const summary =
    (typeof temporalFacet?.summary === "string" && temporalFacet.summary.trim()) ||
...
function latestObservedAt(events: RecentEvent[]): string | undefined {
...
function missingLlmBatchDecision(): ConsolidationBatchDecision {
  return "defer";
}

function factConfirmationKey(preference: MemoryCandidatePreferenceHint): string {
  return `${preference.predicate}:${normalizeText(preference.object)}`;
}

function relationConfirmationKey(relation: MemoryCandidateRelationHint): string {
...
function collectStructuredConfirmations(params: {
  ...
  deltaEvents: RecentEvent[];
  batchMode: boolean;
}): StructuredConfirmationCollection {
  const touchedFactKeys = new Set<string>();
  const touchedRelationKeys = new Set<string>();
  for (const event of params.deltaEvents) {
    ...
    const relations =
      params.ctx.config.advanced.enableGraphPromotion && Array.isArray(hints.relations)
        ? hints.relations
        : hints.relation
          ? [hints.relation]
          : [];
  ...
  const factGroups = new Map<string, PendingFactConfirmation>();
  const relationGroups = new Map<string, PendingRelationConfirmation>();
  for (const event of params.recentEvents) {
    ...
    const preferences =
      allowFactGroups && params.ctx.config.advanced.enableFactPromotion && hints.preference
        ? [hints.preference]
        : [];
  ...
  const pendingFacts = sortPendingGroups(
    [...factGroups.values()].filter((entry) => entry.events.length >= 2),
  );
  const pendingRelations = sortPendingGroups(
    [...relationGroups.values()].filter((entry) => entry.events.length >= 2),
  );
...
function syncSupersededFactBeliefs(store: MemxStoreBundle, ctx: MemoryOperationContext): number {
  let beliefsSuperseded = 0;
  const factBeliefs = store.beliefRepo
    .listByAgent({ agentId: ctx.agentId })
    .filter(
      (belief) =>
        belief.memoryKind === "fact" &&
        belief.contentRef &&
...
export async function runConsolidation(
  ...
  const stats: ConsolidationMaintenanceStats = {
    ...(options.batch
      ? {
          batch: {
            ...options.batch,
  ...
  const promotedMaintenanceMetadata: Array<Record<string, unknown>> = [];
  ...
  try {
    ...
    stats.semanticUpgrade.confirmationGroups.factGroupKeys = pendingFacts.map((entry) =>
      factConfirmationKey(entry.preference),
    );
    stats.semanticUpgrade.confirmationGroups.relationGroupKeys = pendingRelations.map((entry) =>
      relationConfirmationKey(entry.relation),
    );
    ...
    stats.semanticUpgrade.confirmationGroups.selectedFactGroupKeys = selectedFacts.map((entry) =>
      factConfirmationKey(entry.preference),
    );
    stats.semanticUpgrade.confirmationGroups.selectedRelationGroupKeys = selectedRelations.map(
      (entry) => relationConfirmationKey(entry.relation),
    );
    ...
    const confirmationItems = [
      ...selectedFacts.map((entry) => ({
        ...
        structuredSummaries: [
          ...new Set(
            entry.events.slice(0, 4).map((event) => structuredEventEvidenceSummary(event)),
          ),
        ],
    ...
    const factConfirmResults = selectedFacts.map((entry) => {
      ...
      const decision =
        confirmationDecisions?.get(id)?.decision ?? missingLlmBatchDecision();
    ...
    if (!shouldSkipLlmTaskSummaryUpgrade(ctx)) {
      const rankedTasks = store.taskRepo
        ...
        .map((task) => {
          ...
          return {
            ...
            touchedByDelta:
              !options.batch?.lowerWatermarks.task ||
      ...
      stats.budgets.skippedTaskSummaryUpgrades = Math.max(
        0,
        rankedTasks.length - selectedTasks.length,
      );
      if (selectedTasks.length > 0) {
        let didTimeout = false;
        try {
          const llmSummaries = await Promise.race([
            store.reasoner.summarizeTaskEvidenceBatch(
              selectedTasks.map((entry) => entry.evidence),
              {
                stage: "maintenance_async",
                audit: ctx.llmBudgetAudit,
              },
    ...
    for (const { entry, decision } of relationConfirmResults) {
      ...
      const subjectResolution = resolveEntityMention(
        store,
        ctx,
        buildEntityMention({
          ctx,
      ...
      const objectResolution = resolveEntityMention(
        store,
        ctx,
        buildEntityMention({
          ctx,
      ...
      if (subjectResolution.method === "uncertain" || objectResolution.method === "uncertain") {
        stats.semanticUpgrade.skippedReasons.push(
          `graph_endpoint_uncertain:${entry.relation.predicate}`,
        );
        continue;
      }
    ...
    const maintenanceContractDiagnostics =
      summarizeMaintenanceContractDiagnostics(promotedMaintenanceMetadata);
